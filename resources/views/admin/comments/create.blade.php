...
@section('title', trans('global.create_comment'))
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1> {{ trans('global.create_comment') }}</h1>
                </div>
        ...
        <div class="card-body">
            <form action="{{ route('comments.store') }}" method="POST">
                ...
                <div class="form-group {{ $errors->has('comment') ? 'has-error' : '' }}">
                    ...
                    <textarea id="comment" name="comment" class="form-control " required>{{ old('comment', isset($comment) ? $comment->comment_text : '') }}</textarea>
